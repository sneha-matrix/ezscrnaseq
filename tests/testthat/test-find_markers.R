...
test_that("find_markers", {
  ...
  expect_warning(clus_df <- find_markers(sce1, clusters=sce1$Cluster, annot=data.frame(rowData(sce1), row.names=rownames(sce1)), fdr_cutoff=Inf, write=FALSE))
  #expect_equal(rownames(clus_df)[1:3], c("ENSG00000219088", "ENSG00000080815", "ENSG00000258478"))

  expect_warning(clus_df <- find_markers(sce1, clusters=sce1$Cluster, annot=data.frame(rowData(sce1), row.names=rownames(sce1)), fdr_cutoff=Inf, pval.type="all", write=FALSE))
  #expect_equal(rownames(clus_df)[1:3], c("ENSG00000219088", "ENSG00000080815", "ENSG00000258478"))
})
